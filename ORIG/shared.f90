module shared
 ...
 real(kind=dp)                :: kvisc, alpha, T0, Tin, rho0, rho1, eps, T_in_sol
 real(kind=dp)                :: w_eq(0:8), sum1, sum2, sum3, sum4
 real(kind=dp)                :: cx(0:8),cy(0:8)
 real(kind=dp)                :: omega_g, omega_f, tau_g, tau_f, grav
 real(kind=dp), allocatable   :: Nu(:), Nu2(:), Nu3(:),Tavg(:), Tavg2(:), gradTwall(:), gradTwall2(:)
 ...
 real(kind=dp), allocatable   :: react_plus(:,:),react_minus(:,:),k_plus(:,:),k_minus(:,:)
 real(kind=dp)                :: mass, length, temp_aver, T_crit, T_act, T_width,f_plus,f_minus
...
!============================================================================================================
  ...
  character(280)              :: velocity, density, coordinates, temperature, ph_field
  real(kind=dp)               :: sizereal
  integer                     :: sizeint
  integer                     :: ss, ss1, ss2, deltaOut, itOut
  integer                     :: offset_velocity, offset_density, offset_temp, offset_coordinates, &
                                 offset_phasef
